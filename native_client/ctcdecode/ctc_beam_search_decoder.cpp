                   ...
                   int class_dim)
{
  ...
  for (int rel_time_step = 0; rel_time_step < time_dim; ++rel_time_step, ++abs_time_step_) {
    auto *prob = &probs[rel_time_step*class_dim];

    float min_cutoff = -NUM_FLT_INF;
    bool full_beam = false;
    if (has_scorer()) {
      int num_prefixes = std::min<int>(prefixes_.size(), beam_size_);
      std::sort(prefixes_.begin(), prefixes_.begin() + num_prefixes, prefix_compare);
    ...
    for (int index = 0; index < log_prob_idx.size(); index++) {
      ...
      for (int i = 0; i < prefixes_.size() && i < beam_size_; ++i) {
        ...
        if (prefix_new != nullptr) {
          ...
          if (has_scorer() && (c == space_id_ || has_char_level_scorer())) {
    ...
    if (prefixes_.size() >= beam_size_) {
      std::nth_element(prefixes_.begin(),
                       ...
                       prefixes_.end(),
                       prefix_compare);
      for (int i = beam_size_; i < prefixes_.size(); ++i) {
        prefixes_[i]->remove();
      }
...
DecoderState::decode()
{
  ...
  if (has_word_level_scorer()) {
    for (int i = 0; i < beam_size_ && i < prefixes_copy.size(); ++i) {
  ...
  int num_prefixes = std::min<int>(prefixes_copy.size(), beam_size_);
  ...
  for (int i = 0; i < top_paths && i < prefixes_copy.size(); ++i) {
